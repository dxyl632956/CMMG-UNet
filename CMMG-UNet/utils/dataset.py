...
#         print(f"Mode: {mode}, Data length: {len(self.image_list)}") # 建议加上这行打印以确认数据量
...
#         gt_filename = self.image_list[idx]
#         if not gt_filename.startswith('mask_'):
#              gt_filename = 'mask_' + gt_filename
             

#         gt = os.path.join(self.root_path, 'Ground-truths', gt_filename)
...
class QaTa(Dataset):
    ...
    def __init__(self, csv_path=None, root_path=None, tokenizer=None, mode='train', image_size=[224,224]):
        ...
        print(f"Mode: {mode}, Data length: {len(self.image_list)}") 
    ...
    def __getitem__(self, idx):
        ...
        gt_path_a = os.path.join(self.root_path, 'masks', gt_filename)
        

        if gt_filename.startswith('mask_'):
            gt_filename_b = gt_filename
        else:
            gt_filename_b = 'mask_' + gt_filename
        gt_path_b = os.path.join(self.root_path, 'masks', gt_filename_b)


        if os.path.exists(gt_path_a):
            gt = gt_path_a
        elif os.path.exists(gt_path_b):
            gt = gt_path_b
        else:

            print(f"[Warning] Mask file not found: {gt_filename}. Tried: {gt_path_a} and {gt_path_b}")
            gt = gt_path_a 
